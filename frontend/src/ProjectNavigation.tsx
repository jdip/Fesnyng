...
function ProjectDetails({ organization, csrf, project, threads, allProjects, agents, manager, error, selectedName, onOpenThread, onMoveThread, onNewThread, onSaveProject, onLifecycleProject, onWorkspaceOperation }: {
  ...
  return <section className="project-details" aria-label={`${selectedName} project`}>
    ...
    <div className="project-thread-list">{filtered.map((thread) => <ThreadCard key={`${thread.agent}:${thread.session_id}`} active={false}><button className={threadCardTriggerClassName('pe-44')} onClick={() => onOpenThread(thread.agent, thread.session_id)}><ThreadCardContent title={<span title={thread.title}>{thread.title}</span>} subtitle={agents.find((agent) => agent.id === thread.agent)?.name ?? thread.agent} /></button><ThreadCardActions><select className="max-w-40 rounded-md border border-[#d8dfd5] bg-white p-1.5 text-xs text-[#394b38]" aria-label={`Project for ${thread.title}`} value={thread.project_id ?? ''} onChange={(event) => { void onMoveThread(thread, event.target.value || null); }}><option value="">Ungrouped</option>{(project?.archived ? [project, ...allProjects] : allProjects).map((item) => <option key={item.id} value={item.id} disabled={item.archived}>{item.name}{item.archived ? ' (archived)' : ''}</option>)}</select></ThreadCardActions></ThreadCard>)}{!filtered.length && <p className="muted">No matching threads.</p>}</div></>}
