...
vi.mock('./Conversation', async () => {
  ...
  const { WorkspaceInspection } = await import('./WorkspaceInspection');
  return { Conversation: ({ sessionId, threadListTarget, onThreadSelect, onSessionChange, readOnly, executionBlocked, executionBlockedState, workspace }: { sessionId?: string; threadListTarget?: HTMLElement; onThreadSelect?: () => void; onSessionChange?: (id: string) => void; readOnly?: boolean; executionBlocked?: boolean; executionBlockedState?: string; workspace?: { organization: string; agent: string; csrf: string; revision?: number; onChanged?: (state: 'ready' | 'removed' | 'legacy' | 'unavailable' | 'removing' | 'replacing' | undefined) => void } }) => {
    const [draft, setDraft] = useState('');
    return <>{workspace && sessionId && <WorkspaceInspection organization={workspace.organization} agent={workspace.agent} session={sessionId} csrf={workspace.csrf} revision={workspace.revision} onChanged={workspace.onChanged} />}<p>Native conversation {sessionId}</p>{readOnly ? <p>This thread is permanently frozen and read-only.</p> : executionBlocked ? <p>{executionBlockedState === 'removed' ? 'This workspace was removed. Prepare its replacement before continuing.' : 'This workspace cannot run until its state is resolved. Inspect the workspace before continuing.'}</p> : <textarea aria-label="Composer draft" value={draft} onChange={(event) => setDraft(event.target.value)} />}{threadListTarget && createPortal(<><button onClick={onThreadSelect}>Open sidebar thread</button><button onClick={onThreadSelect}>Create sidebar thread</button><button onClick={() => onSessionChange?.('other-thread')}>Select other sidebar thread</button></>, threadListTarget)}</>;
  } };
});
