...
vi.mock('./ThreadArtifact', () => ({
  ThreadArtifactPanel: ({ session, onClose }: { session: { id: string; title: string }; onClose: () => void }) => <aside aria-label="Workspace files"><h2>{session.title}</h2><p>{session.id}</p><button onClick={onClose}>Close files</button></aside>,
}));
...
test('opens files for a non-selected thread without replacing the conversation or its draft', async () => {
