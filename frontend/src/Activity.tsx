...
export function Activity({ organization, agents, onOpen, compact = false }: { organization: string; agents: Agent[]; onOpen: (agent: string, session: string) => void; compact?: boolean }) {
  ...
  useEffect(() => {
    ...
    async function poll() {
      ...
      const meaningful = next.filter((item) => ['completed', 'failed', 'unresolved', 'uncertain', 'needs input'].includes(item.state));
      const changes = meaningful.filter((item) => previous ? previous.get(item.id) !== item.state : item.state === 'needs input' || item.state === 'unresolved' || item.state === 'uncertain');
      ...
      previous = new Map(next.map((item) => [item.id, item.state]));
      setItems(next.sort((a, b) => (a.state === 'needs input' ? -1 : b.state === 'needs input' ? 1 : b.updated - a.updated))); setErrors(failed);
