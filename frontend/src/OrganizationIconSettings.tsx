...
type IconState = { organization: string; saved: Icon | null; draft: Icon | null; loading: boolean; busy: boolean; error: string; notice: string };
...
export function OrganizationIconSettings({ organization, csrf, onSaved }: {
  ...
  const [state, setState] = useState<IconState>({ organization, saved: null, draft: null, loading: true, busy: false, error: '', notice: '' });
  ...
  const current = visible ? state : { organization, saved: null, draft: null, loading: true, busy: false, error: '', notice: '' };
  ...
  useEffect(() => {
    ...
    void api<unknown>(`/organizations/${encodeURIComponent(organization)}`, { signal: controller.signal }).then((value) => {
      ...
      setState({ organization, saved: updated.icon ?? null, draft: updated.icon ?? null, loading: false, busy: false, error: '', notice: '' });
    }).catch((cause: unknown) => {
      if (controller.signal.aborted || activeOrganization.current !== organization) return;
      setState({ organization, saved: null, draft: null, loading: false, busy: false, error: errorMessage(cause), notice: '' });
    });
    return () => {
      controller.abort();
  ...
  const loadImage = (file: File | undefined) => {
    ...
    if (!IMAGE_TYPES.has(file.type)) {
      setState((previous) => previous.organization === organization ? { ...previous, error: 'Choose a PNG, JPEG, or WebP image.', notice: '' } : previous);
      return;
    }
    if (file.size > MAX_IMAGE_BYTES) {
      setState((previous) => previous.organization === organization ? { ...previous, error: 'Choose an image no larger than 2 MiB.', notice: '' } : previous);
      return;
    }
    ...
    reader.onload = () => {
      ...
      if (version !== readerVersion.current || activeOrganization.current !== organization || typeof value !== 'string' || !validImageDataUrl(value)) return;
      setState((previous) => previous.organization === organization
        ? { ...previous, draft: { kind: 'image', value }, error: '', notice: '' }
        : previous);
    };
    reader.onerror = () => {
      if (version !== readerVersion.current || activeOrganization.current !== organization) return;
      setState((previous) => previous.organization === organization ? { ...previous, error: 'The image could not be read.', notice: '' } : previous);
    };
  ...
  const save = async () => {
    ...
    try {
      ...
      if (controller.signal.aborted || activeOrganization.current !== organization || updated.id !== organization) return;
      setState({ organization, saved: updated.icon ?? null, draft: updated.icon ?? null, loading: false, busy: false, error: '', notice: 'Organization icon saved.' });
      onSaved(updated);
    } catch (cause) {
      if (controller.signal.aborted || activeOrganization.current !== organization) return;
  ...
  return <section className="app-panel app-form" aria-label="Organization identity"><h3>Organization identity</h3><p className="muted">Choose an emoji or a small image for this organization.</p>
    ...
    <div className="app-actions"><button type="button" className="app-button primary" disabled={current.busy} onClick={() => { void save(); }}>{current.busy ? 'Saving…' : 'Save icon'}</button><button type="button" className="app-button" disabled={current.busy || current.draft === null} onClick={() => setDraft(null)}>Clear icon</button><button type="button" className="app-button quiet" disabled={current.busy} onClick={() => setDraft(current.saved)}>Reset</button></div>
    ...
    {current.error && <button type="button" className="app-button quiet" onClick={() => setReload((value) => value + 1)}>Retry identity</button>}
  </section>;
}
