...
export function projectGroupingWarning(value: unknown): ProjectGroupingWarning | undefined {
  ...
  return record as ProjectGroupingWarning;
}

export function publishProjectGroupingWarning(value: unknown) {
  const warning = projectGroupingWarning(value);
  if (warning) window.dispatchEvent(new CustomEvent<ProjectGroupingWarning>('fesnyng-project-grouping-warning', { detail: warning }));
}
