...
export function AgentConversation({ organization, agent, csrfToken, viewerId = '', hidden, refreshKey, workspaceInspectionRevision = 0, workspaceInspectionState, projectRevision = 0, sessionId, threadListTarget, newThreadRequest, creation, createdGroupingWarning, onGroupingWarningResolved, onNewThreadStarted, onNewThreadFailed, threadPageSize, onThreadSelect, onSessionChange, onTitleChanged, onError, onOpen }: { organization: string; agent: Agent; csrfToken: string; viewerId?: string; hidden: boolean; refreshKey: number; workspaceInspectionRevision?: number; workspaceInspectionState?: WorkspaceUpdate['inspection']['state']; projectRevision?: number; sessionId: string | undefined; threadListTarget: HTMLElement | null; newThreadRequest: number | undefined; creation?: NativeThreadCreation; createdGroupingWarning?: ProjectGroupingWarning; onGroupingWarningResolved?: () => void; onNewThreadStarted: (request: number) => void; onNewThreadFailed?: (request: number, cause: unknown) => void; threadPageSize: number; onThreadSelect: () => void; onSessionChange: (id: string | undefined) => void; onTitleChanged?: () => void; onError: (cause: unknown) => void; onOpen: (id: string, session?: string) => void }) {
  ...
  return <ConversationDeliveryProvider organization={organization} agent={agent.id} session={sessionId} csrf={csrfToken} onOpen={onOpen} viewerId={viewerId} readOnly={readOnly || executionBlocked}>{shell}</ConversationDeliveryProvider>;
}
