...
export function AgentConversation({ organization, agent, csrfToken, hidden, refreshKey, sessionId, threadListTarget, newThreadRequest, projectId, onNewThreadStarted, threadPageSize, onThreadSelect, onSessionChange, onError, onOpen }: { organization: string; agent: Agent; csrfToken: string; hidden: boolean; refreshKey: number; sessionId: string | undefined; threadListTarget: HTMLElement | null; newThreadRequest: number | undefined; projectId?: string | null; onNewThreadStarted: (request: number) => void; threadPageSize: number; onThreadSelect: () => void; onSessionChange: (id: string | undefined) => void; onError: (cause: unknown) => void; onOpen: (id: string, session?: string) => void }) {
  ...
  useEffect(() => {
    ...
    return () => controller.abort();
  }, [agent.id, organization, refreshKey]);
